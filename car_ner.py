...
from utils import get_logger, save_model, test_ner, load_config, get_lines
from data_utils import load_data, create_maps, prepare_dataset, make_path, clean
from data_utils import load_word2vec, input_from_line, BatchManager
from data_helpers import DataHelpers
from data_annotate import Annotate
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        with open("temp/all_data_clean.txt", "r", encoding="utf-8") as f:
            for index, line in enumerate(f):
                ...
                result = model.evaluate_line(sess, input_from_line(line, word_to_id), id_to_tag)
                entities_pre = list(set([item["word"] for item in result["entities"]]))
...
if __name__ == "__main__":
    os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"  # 设置提示信息的等级
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"  # CUDA设备顺序
    os.environ['CUDA_VISIBLE_DEVICES'] = '0'  # 指定GPU（0，1，2）
    main()
